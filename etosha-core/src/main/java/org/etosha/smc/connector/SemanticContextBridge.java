...
public class SemanticContextBridge  {
	...
	/**
	 * creates a new page in Semantik-Wiki if there is not such a page and calculates the pagename
	 * which is used later in the procedure of logging data.
	 * 
	 * @param localUser2
	...
	public void logFileToHistoryToPage(String fn) throws Exception {
		File f = new File(fn);
		if ( !f.canRead() ) throw new Exception("File: " + f.getAbsolutePath() + " not readable!!!" );
