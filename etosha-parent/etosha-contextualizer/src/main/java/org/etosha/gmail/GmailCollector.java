...
public class GmailCollector {
    ...
    public String APP_NAME = "Gmail API Quickstart";
    ...
    public GoogleCredential credential = null;
    ...
    public GmailCollector() throws IOException, URISyntaxException, Exception {

        boolean testQueryImport = false;
        boolean testThreadImport = true;
        ...
        File f = new File( ClientSecretFileManager.getPathName() );

        if ( f != null )
            System.out.println( "> client secret file : " + f.getAbsolutePath() );
        else throw new Exception( "No client secret for GmailCollector.");
...
//-------------------- moved to UI -----        
    ...
    public List<Message> listMessagesWithLabels(List<String> labelIds) throws IOException {
        ...
        if( labelIds.size() > 1 ) return listMessagesWithLabelsSPLITTED( labelIds );
        
        
        
        System.out.println( labelIds.size() + " LABEL IDS had been chosen." );
        ...
        System.out.println("> listMessagesWithLabels() ... " + messagesFull.size());
  ...
  public MimeMessage getMimeMessage(String messageId)
    ...
    private List<Message> listMessagesWithLabelsSPLITTED(List<String> labelIds) throws IOException {
        
        List<Message> mAll = new ArrayList<Message>();
        int z = 0;
        for( String lid : labelIds ) {
            System.out.println( "*** Iteration *** "  + z );
            List<String> l = new ArrayList<String>();
            String id = GmailLabelSelectorFrame.labels.get(lid);
            //if ( id.startsWith("Best Pracrtise")||id.startsWith("FAQ")||id.startsWith("Tea") ) {
            if ( isNotExcluded( id ) ) {
                l.add( lid );

                List<Message> m = listMessagesWithLabels(l);
                mAll.addAll(m);
                z++;
            }
        }
    ...
    private boolean isNotExcluded(String id) {
        boolean isExcluded = GmailLabelSelectorFrame.excludeLabelsV.contains(id);
        return !isExcluded;
    }
