 ...
 * @author kamir
 */
public class FlowDescriptorDEMO {

    
  public static void main(String[] args) throws IOException {
    
    String RDF_OutputLocation = "./data/rdf/demo_flow_1.rdf";
    
    System.out.println("The RDF outputpath is: " + RDF_OutputLocation + ")");

    FlowDescriptorDEMO demo = null;

    try { 
        
      demo = new FlowDescriptorDEMO(RDF_OutputLocation);
   ...
   */
  FlowDescriptorDEMO(String modelFileName) throws IOException {
    ...
    FlowContextualizer.createDataset( m, "tweetsTTIP_CETA" );  
    
    // etosha create datasource flumeAgent01; // defines an entity (as page)
    FlowContextualizer.createDatasource( m, "flumeAgent01" );  
    
    // etosha link tweetsTTIP_CETA ‘hasSource’ flumeAgent01; // links S and O via P 
    FlowContextualizer.linkDatasetToDatasource( m, "tweetsTTIP_CETA", "flumeAgent01" );  
    ...
    FlowContextualizer.addTagToDatasetToColumn( m, "tweetsTTIP_CETA", column, key, value );  

    
    // etosha add tag to dataset tweetsTTIP_CETA isAbout TTIP
    key="isAbout";
    value="CETA";
    FlowContextualizer.addTagToDataset( m, "tweetsTTIP_CETA", key, value );  
